...
class PedidoAprovacaoForm extends TPage
{
    ...
    function __construct($param = null)
    {
        ...
        $pedido = MaterialOrders::find($param['id']);
        TTransaction::close();
    ...
    public function onEdit($param = null)
    {
        try {
            if (isset($param['key'])) {
                TTransaction::open('bancodados');
                $pedido = MaterialOrders::find($param['key']);
                $this->form->setData($pedido); //inserindo dados no formulario. 

                $pivot = PivotMaterialOrders::where('id_pedido_material', '=', $pedido->id)
                    ->load();
        ...
        try {
            ...
            if ($param['status'] == "PENDENTE") {
                ...
                if (isset($param["id"]) && !empty($param["id"])) {
                    $pedido = new MaterialOrders($param["id"]);
                ...
                PivotMaterialOrders::where('id_pedido_material', '=', $pedido->id)->delete();
